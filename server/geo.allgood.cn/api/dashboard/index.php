...
function geo_dashboard_asset_map(PDO $pdo, int $cloudUserId, array $pairs): array {
    ...
    $sql = 'SELECT install_id,local_result_id,public_url,file_size,updated_at FROM geo_sync_assets WHERE cloud_user_id=? AND kind="screenshot" AND (' . implode(' OR ', $clauses) . ') ORDER BY updated_at DESC';
