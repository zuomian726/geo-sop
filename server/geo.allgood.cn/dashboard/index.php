...
<div class="app-launch-notice" id="appLaunchNotice" role="status" aria-live="polite">
    <div class="app-launch-copy">
        <strong id="appLaunchTitle">正在打开 GEO-SOP</strong>
        <span id="appLaunchDetail">正在把请求交给本机桌面应用。</span>
        <div class="app-launch-actions" id="appLaunchActions">
            <button type="button" onclick="retryLocalApp()">再次尝试</button>
            <a class="btn primary" href="/#download">下载最新版</a>
        </div>
    </div>
    <button type="button" class="btn small" aria-label="关闭提示" onclick="hideAppLaunchNotice()">关闭</button>
</div>
...
<main class="wrap">
    ...
    <section class="tabs" id="insights" data-collapsible="insights" data-collapse-default="open">
        <nav class="tab-nav">
            ...
            <a href="#reference-ranking">引用参考源分析</a>
            <a href="#reference-trends">引用参考源走势图</a>
...
<div class="result-modal" id="resultDetailModal" role="dialog" aria-modal="true" aria-labelledby="resultDetailTitle" onclick="if(event.target===this) closeResultDetail()">
    <div class="result-dialog">
        <div class="result-dialog-head">
            <div><span class="kicker">Answer detail</span><h2 id="resultDetailTitle">回答详情</h2></div>
            <button class="result-close" type="button" aria-label="关闭回答详情" onclick="closeResultDetail()">&times;</button>
        </div>
        <div class="result-dialog-body" id="resultDetailBody"><div class="empty">正在加载完整回答...</div></div>
    </div>
</div>
    ...
    try {
        ...
        rows.innerHTML = data.results.map(function(item){
            ...
            return '<tr>' +
                ...
                '<td>' + refs + '</td>' +
                '<td>' + shot + '</td>' +
                '<td><div class="answer-snippet">' + h(answer || '-') + '</div><button class="text-action" type="button" data-install-id="' + h(item.install_id) + '" data-local-id="' + h(item.local_id) + '" onclick="openResultDetail(this.dataset.installId,this.dataset.localId)">查看完整回答</button></td>' +
            '</tr>';
        }).join('');
    } catch (e) {
...
function safeResultUrl(value, allowRelative){
    try {
        var raw = String(value || '').trim();
        if (!raw) return '';
        if (allowRelative && raw.charAt(0) === '/') return new URL(raw, window.location.origin).href;
        if (!/^[a-z][a-z0-9+.-]*:\/\//i.test(raw) && raw.indexOf('.') > 0) raw = 'https://' + raw;
        var url = new URL(raw);
...
function closeResultDetail(){
    document.getElementById('resultDetailModal').classList.remove('show');
    document.body.style.overflow = '';
}

async function openResultDetail(installId, localId){
    var modal = document.getElementById('resultDetailModal');
    var body = document.getElementById('resultDetailBody');
    modal.classList.add('show');
    document.body.style.overflow = 'hidden';
    body.innerHTML = '<div class="empty">正在加载完整回答...</div>';
    try {
        var data = await geoApi({action: 'result', install_id: installId, local_id: String(localId)});
        var item = data.result || {};
        var keywords = Array.isArray(item.exposed_keywords) && item.exposed_keywords.length ? item.exposed_keywords.join('、') : '未命中';
        var refs = Array.isArray(item.references) ? item.references : [];
        var refHtml = refs.map(function(ref){
            var rawUrl = ref.url || ref.link || '';
            var url = safeResultUrl(rawUrl);
            var label = ref.title || ref.name || ref.domain || rawUrl || '引用来源';
            return url ? '<a href="' + h(url) + '" target="_blank" rel="noopener noreferrer"><strong>' + h(label) + '</strong><br><span class="muted">' + h(url) + '</span></a>' : '';
        }).filter(Boolean).join('');
        var sentiment = item.ai_sentiment || {};
        var sentimentLabel = sentiment.sentiment || sentiment.label || sentiment.overall_sentiment || '';
        var sentimentReason = sentiment.reason || sentiment.analysis || sentiment.summary || '';
        var screenshotUrl = safeResultUrl(item.screenshot_url, true);
        body.innerHTML =
            '<div class="result-detail-meta">' +
                '<div><span>任务</span><strong>' + h(item.task_name || ('#' + item.local_task_id)) + '</strong></div>' +
                '<div><span>AI 平台</span><strong>' + h(item.platform_name || item.platform || '-') + '</strong></div>' +
                '<div><span>品牌曝光</span><strong>' + (item.has_brand_exposure ? '已曝光' : '未曝光') + '</strong></div>' +
                '<div><span>采集时间</span><strong>' + h(item.created_at || '-') + '</strong></div>' +
            '</div>' +
            '<div class="result-section"><h3>监测问题</h3><div class="result-answer">' + h(item.question || '-') + '</div></div>' +
            '<div class="result-section"><h3>完整回答</h3><div class="result-answer">' + h(item.answer || '暂无回答内容') + '</div></div>' +
            '<div class="result-section"><h3>品牌命中</h3><p>' + h(keywords) + '</p></div>' +
            (sentimentLabel || sentimentReason ? '<div class="result-section"><h3>智慧舆情</h3><p><strong>' + h(sentimentLabel || '已分析') + '</strong>' + (sentimentReason ? '<br><span class="muted">' + h(sentimentReason) + '</span>' : '') + '</p></div>' : '') +
            '<div class="result-section"><h3>引用来源（' + refs.length + '）</h3><div class="result-reference-list">' + (refHtml || '<div class="empty">当前回答没有引用来源。</div>') + '</div></div>' +
            (screenshotUrl ? '<div class="result-section"><a class="btn primary" href="' + h(screenshotUrl) + '" target="_blank" rel="noopener noreferrer">查看截图留证</a></div>' : '');
    } catch (error) {
        body.innerHTML = '<div class="empty">' + h(error.message || '回答详情加载失败') + '</div>';
    }
}
...
var appLaunchAttempt = 0;
var appLaunchTarget = 'dashboard';

function setAppLaunchNotice(state, title, detail){
    var notice = document.getElementById('appLaunchNotice');
    if (!notice) return;
    notice.className = 'app-launch-notice show ' + (state || 'opening');
    document.getElementById('appLaunchTitle').textContent = title;
    document.getElementById('appLaunchDetail').textContent = detail;
}

function hideAppLaunchNotice(){
    var notice = document.getElementById('appLaunchNotice');
    if (notice) notice.className = 'app-launch-notice';
}
...
function openLocalApp(target){
    appLaunchTarget = target || 'dashboard';
    var attempt = ++appLaunchAttempt;
    var handedToSystem = false;
    var markHandedOff = function(){ handedToSystem = true; };
    var onBlur = function(){ markHandedOff(); };
    var onPageHide = function(){ markHandedOff(); };
    var onVisibility = function(){ if (document.hidden) markHandedOff(); };
    ...
    setAppLaunchNotice('opening', '正在打开 GEO-SOP', '正在把请求交给本机桌面应用，请稍候。');
    window.location.href = 'geo-sop://open?target=' + encodeURIComponent(appLaunchTarget);
    setTimeout(function(){
        cleanup();
        if (attempt !== appLaunchAttempt) return;
        if (handedToSystem || document.hidden) {
            setAppLaunchNotice('success', '已发送打开请求', '系统已接收 GEO-SOP 打开请求，请切换到桌面应用继续操作。');
            setTimeout(function(){ if (attempt === appLaunchAttempt) hideAppLaunchNotice(); }, 3200);
            return;
        }
        setAppLaunchNotice('warning', '没有检测到桌面应用', '请确认已安装最新版 GEO-SOP。安装后再次尝试，并在 App 内登录同一个云端账号。');
    }, 1800);
}
...
function setActiveDashboardNav(id){
    document.querySelectorAll('.tab-nav a[href^="#"]').forEach(function(link){
...
function openDashboardTarget(id, updateHistory){
    ...
    if (!target) return;
    var section = target.matches('[data-collapsible]') ? target : target.closest('[data-collapsible]');
    if (section && section.classList.contains('is-collapsed')) {
        var toggle = section.querySelector('.collapse-toggle');
        if (toggle) toggle.click();
    }
    setActiveDashboardNav(id);
...
function initDashboardNavigation(){
    document.querySelectorAll('a[href^="#"]').forEach(function(link){
        link.addEventListener('click', function(event){
            ...
            event.preventDefault();
            openDashboardTarget(id, true);
        });
    });
    window.addEventListener('hashchange', function(){
        openDashboardTarget(decodeURIComponent(window.location.hash.slice(1)), false);
    });
