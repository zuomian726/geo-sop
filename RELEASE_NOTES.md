...
- 完成可商用的品牌检测与舆情分析工作流：任务创建、平台登录检测、AI 回答采集、截图留证、品牌曝光、引用来源、GEO 稿件引用和精炼 KPI 看板。
- 桌面端与云端使用同一账号体系，通过 HTTPS API 与可撤销 Token 双向同步任务、回答、统计、截图和分析数据；新安装登录后可恢复历史记录。
- 云端可无刷新创建任务，由同账号在线客户端自动领取、执行和回传；心跳、状态机、失败原因、多账号隔离及空库首次部署均纳入验收。
- 看板分析和单条智慧舆情统一支持 OpenAI 兼容与 Anthropic 兼容接口，自动补全服务路径，并在保存时校验 base_url、Key 和模型；包含超时反馈、配置脱敏、关键词降级与 AI 辅助下一步动作。
- 云端结果库增加可重复的 V1 索引迁移，旧数据库会自动补齐账号、任务、平台、曝光和时间组合索引；全新部署可从空库直接完成注册、Demo、同步、远程任务和导出。
- Windows 提供内置 Python 运行时与 Chromium 的一键 EXE 安装包；macOS 分别提供 Apple Silicon 与 Intel DMG，安装和升级保留本地数据库与浏览器登录状态。
- 客户端每 30 分钟检查正式更新，同一版本只提醒一次；安装包必须通过 HTTPS 同域、SHA-256 和系统签名验证后才能安装。
- 在线 Demo 使用独立合成数据和只读安全限制，可查看、筛选、导出和体验完整分析界面，不会修改真实客户数据。
- V1 自动化测试、真实浏览器、多账号隔离数据库、客户端/云端同步和发布前安全门槛均记录在 `V1_ACCEPTANCE.md`。

## v0.3.44-dev - 2026-07-16

- 空闲客户端的云端任务轮询由固定 10 秒改为自适应策略：等待任务时退避到 60 秒，排队、采集和补传期间保持快速轮询。
- 心跳接口会返回账号是否存在待领取任务；客户端收到通知后立即唤醒任务线程，云端下发任务无需等待完整空闲轮询周期。
- 独立心跳继续保持 20 秒频率和 90 秒离线判定，预计单个空闲客户端请求量降低约 56%，同时任务接收延迟保持在约 20 秒以内。
- 新协议向后兼容，旧客户端会忽略新增心跳字段并继续正常工作。
- 93 项自动化测试通过。

## v0.3.43-dev - 2026-07-16

- 修复重装或本地数据库重置后，残留云端游标可能导致历史采集结果没有完整恢复的问题；空工作区登录时会从头安全合并账号历史。
- 云端历史恢复分页只在第一页返回任务、稿件和智慧舆情配置，后续页仅传输结果数据，减少大数据账号首次同步时的重复查询与网络传输。
- 服务端保持对旧客户端的响应兼容，未携带新分页参数的版本仍按原协议返回完整元数据。
- 91 项自动化测试通过。

## v0.3.42-dev - 2026-07-16

- 在线 Demo 中创建任务、平台登录、AI 配置和本机执行入口统一为可解释的只读交互，不再有的静默禁用、有的尝试唤起本机 App。
- 点击受限功能会显示非阻塞说明，明确 Demo 不会连接用户的本机账户、浏览器或桌面应用，并提供桌面版使用路径。
- 正式云端账号仍可正常唤起桌面端，服务端对 Demo 的写入、任务下发和资源上传保护保持不变。
- 89 项自动化测试通过。

## v0.3.41-dev - 2026-07-16

- 云端与在线 Demo 的“引用参考源分析”和“引用参考源走势图”现在拥有独立导航目标，不再点击后落到同一位置。
- 点击位于收起区域内的导航目标时，会先自动展开父区块，再准确滚动到对应内容。
- 工作台导航高亮会跟随当前点击的功能，浏览器前进、后退和直接打开带锚点链接时也会恢复正确位置。
- 固定顶部栏下的模块增加滚动留白，标题不再被顶部栏遮挡。
- 88 项自动化测试通过。

## v0.3.40-dev - 2026-07-16

- 云端“打开本机 App”改为非阻塞连接提示，不再在成功唤起后仍弹出失败警告。
- 唤起过程会监听页面失焦、隐藏和系统页面切换；未检测到桌面应用时提供再次尝试和最新版下载入口。
- 从云端点击“检测平台登录”会让桌面端直接打开平台登录检测窗口，不再只停留在工作台首页。
- 智慧舆情深度链接会直接进入对应设置标签，减少云端到本机的重复导航。
- Windows 安装器与 macOS App 的 `geo-sop://` 协议注册纳入自动化契约测试，未知目标会被限制在本地工作台内。
- 88 项自动化测试通过。

## v0.3.39-dev - 2026-07-16

- 创建任务与智慧舆情长表单适配桌面端最小窗口，操作按钮不再落到屏幕之外。
- 长表单改为固定标题和底部操作区，正文独立滚动，调整参数时不再需要滚动整个页面。
- 弹窗宽度增加响应式约束，在浏览器模式和较窄窗口中仍保持完整可操作。
- 移除会创建 `admin/admin` 的遗留开发命令，改为密码隐藏、二次确认且执行输入校验的安全账户创建命令。
- 使用隔离云端账户完成首次登录、历史合并、云端在线状态和空看板真实浏览器验证，无控制台或网络错误。
- GEO 稿件被引用分析按安装实例和主域名预索引候选稿件，AIVA 生产数据查询从约 11 秒降低到约 2 秒，结果完全一致。
- 83 项自动化测试及生产桌面/云端 8 步完整流水线通过。

## v0.3.38-dev - 2026-07-16

- 客户端心跳从任务拉取、历史合并和结果上传线程中独立出来，慢同步不再阻塞在线状态上报。
- 登录成功后立即唤醒心跳与云端任务线程，不再等待同步网络请求完成才进入工作台。
- 心跳使用更短的连接与响应超时，网络波动时能够更快进入下一轮自动重试。
- 客户端连接状态新增独立心跳线程健康度，可识别任务线程正常但保活线程失效的异常情况。
- 服务端数据库结构变更改为带版本和并发锁的一次性迁移，高频登录、心跳、同步与查询不再反复执行建表或改表。
- 77 项自动化测试通过，生产心跳、同步、资源上传、Demo 和云端查询完成回归验证。

## v0.3.37-dev - 2026-07-16

- 空账号数据看板改为紧凑的三步首次引导，不再展示大面积无意义的空图表。
- 引用分析、引用走势、域名列表和 GEO 稿件分析改为首次打开对应标签时加载，减少客户端首屏请求和等待时间。
- 修复 Element Plus 标签点击事件名称读取错误，确保按需加载后每个分析标签仍能正常查询。
- 图表在无数据或页面卸载时统一释放，避免账号切换和长时间使用产生残留实例。
- 端云任务状态增加不可回退保护，多客户端并发领取、幂等完成和延迟状态请求均通过线上验收。
- 多设备同步继续采用非破坏式合并，空本地库不会删除云端或其他设备的历史数据。

## v0.3.36-dev - 2026-07-16

- 修复云端状态请求短暂失败后，客户端一直显示“同步异常”的粘滞状态问题。
- 新增“立即重连”，可以唤醒后台同步线程并马上重试心跳，无需重启客户端。
- 心跳中断、登录失效、网络超时现在会显示不同的可操作提示；云端状态超时不再阻断本地看板。
- 后台工作器新增心跳过期状态与主动唤醒机制，自动同步与远程任务恢复更清晰稳定。
...
- Windows 构建同步使用固定版 Playwright 的兼容安装命令，内置 Chromium、原生 EXE 启动、用户目录配置和安装器完整性均在 CI 中通过验证；自动回归增加至 60 项。
- 云端登录、注册、鉴权、同步 API、Demo 与全部网页静态资源纳入同一 Git 版本，生产数据库和第三方凭据继续保留在仓库外的私有配置中。
- 移除服务端自动创建默认账号的遗留逻辑，Web 会话启用 Secure、HttpOnly、SameSite 和严格会话模式；暂不使用的短信与微信入口默认关闭。
- 新增公网云端冒烟测试，覆盖 Demo 登录、六类数据查询、Excel 导出和只读任务保护；完整自动回归增加至 67 项。
