"""
数据库迁移脚本：添加 schedule_enabled 字段
"""
import sys
import os
...
def migrate():
    ...
    with engine.connect() as conn:
        ...
        if 'schedule_enabled' not in columns:
            ...
            conn.execute(text("ALTER TABLE monitor_tasks ADD COLUMN schedule_enabled BOOLEAN DEFAULT 0"))
            conn.commit()
            print("已添加 schedule_enabled 字段")
        else:
            print("schedule_enabled 字段已存在")
        
        # 确保现有任务有正确的默认值
        conn.execute(text("UPDATE monitor_tasks SET schedule_enabled = 0 WHERE schedule_enabled IS NULL"))
        conn.commit()
